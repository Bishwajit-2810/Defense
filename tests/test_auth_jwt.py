...
The API-key path itself is unchanged MVP behaviour (any non-empty key
authenticates, §5.6 P1) — these tests pin the boundary between the two paths.
"""
...
async def test_valid_token_via_query_param_authenticates_as_itself():
    ...
    principal = await get_current_user(
        credentials=None,
        api_key=None,
        api_key_query=_token(tenant_id="acme"),
    )
...
async def test_expired_token_via_query_param_is_rejected():
    ...
    with pytest.raises(HTTPException) as exc:
        await get_current_user(credentials=None, api_key=None, api_key_query=expired)
...
async def test_forged_token_via_query_param_is_rejected():
    ...
    with pytest.raises(HTTPException) as exc:
        await get_current_user(credentials=None, api_key=None, api_key_query=forged)
    assert exc.value.status_code == 401
...
async def test_expired_token_via_api_key_header_is_rejected():
    ...
    with pytest.raises(HTTPException) as exc:
        await get_current_user(
            credentials=None, api_key=_token(expires_in_hours=-1), api_key_query=None
        )
    assert exc.value.status_code == 401
...
async def test_plain_api_key_still_works():
    ...
    principal = await get_current_user(
        credentials=None, api_key=None, api_key_query="demo"
    )
...
async def test_client_cannot_set_auth_method_claim():
    ...
    principal = await get_current_user(
        credentials=None, api_key=None, api_key_query=hostile
    )
    assert principal["auth_method"] == "jwt"
