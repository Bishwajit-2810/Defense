...
| **Regression coverage** | **1,420 tests collected across 68 files; 1,417 pass, 3 skipped in 62 s** (29 Aug 2026 — the 3 are the `e2e`/`destructive` markers a plain run excludes). Every finding in PROJECT_ASSESSMENT that was fixed has a test that fails if it regresses — which is the property that matters more than the count. |
