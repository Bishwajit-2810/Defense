...
**Scale (measured 29 Aug 2026):** **58,475 lines of Python across 185 files** —
`src/defense/services/` 23,698, `tests/` 19,597, `src/defense/libs/` 5,707,
`src/defense/mcp_servers/` 3,377, `eval/` 2,402, `run_all.py` 856,
`src/defense/contracts/` 153. **1,420 tests** across 68 test files (1,417 pass,
3 skipped — the `e2e`/`destructive` markers). Dashboard: **96** unit tests across
...
*(Was 55,965 lines / 1,396 tests on 23 Aug; 51,801 / 1,315 on 20 Aug;
49,341 / 1,261 on 17 Aug; 27,537 / 673 on 5 Aug.)*
...
| **Stop / resume / delete a job** | A job is N envelopes across four streams, not a process, so **stop** is a Redis flag (`libs/jobs.py`) that ingestion, Stage 1, the router and Stage 2 each check as they pick a message up — the cost of a stop is one post per stage, not the rest of the batch, and `cancelled` is terminal so no late landing can reopen it. **Resume** re-queues *only* what an interrupted job never finished, derived from Postgres alone (the Redis counters do not survive the power cut that makes resume necessary) and rebuilding them with `completed` seeded, so a job cut off at 30/300 continues at 30/300. A post counts as finished only when its stored result is **stamped with that job's id** — `analysis_results` is upserted per post, so the timestamp test this used to rely on marked a job `done` whose post was still in Stage 2, because a *different* run of the same post had landed in the meantime. **Delete** removes the job record, never the posts' `analysis_results` — those are keyed by post and written by several jobs. Measured against the live stores on the 50-post corpus: 29 posts already analysed → **21 re-enqueued, 0 of the 29 repeated**, counters rebuilt to 29/50; a job whose last progress write was <5 min ago is refused with a 409. 42 regression tests, `tests/test_job_lifecycle.py`. | ✅ Measured |
...
| **Per-post live trace** | Streams per-stage events over SSE, including explicit `skipped` frames when Stage 2 is bypassed and the routing gate's own inputs next to its verdict. A live per-post trace during a defense is worth a lot. Traceable **by post id**, not only from the recent-50 dropdown. The session lives outside React (`utils/traceSession.js`), so leaving the tab no longer abandons the post mid-pipeline and a reload re-attaches to the job — [DASHBOARD_UI.md](DASHBOARD_UI.md) §6.1. | ✅ Measured |
| **Re-run one post** | A single row can be re-analysed from the Posts tab (`POST /v1/analysis/run` with one `post_ids` entry and `want_summary: true`, which forces it through Stage 2 — the stage that writes the summary). The remedy for a post whose Summary cell reads "no summary" used to be re-uploading the entire JSON file. | ✅ Measured |
| **Job-scoped reports** | The Jobs tab's per-row download reports on *that job's posts*, via `export_latest?job_id=…`. Campaign was previously the only scope a report understood, and a job run from explicit post ids has no campaign — so a one-post job downloaded a whole-corpus PDF. The scope is now filtered in every aggregate, keyed into the report cache, and printed on the document — [REPORTS.md](REPORTS.md) §1.1. | ✅ Measured |
