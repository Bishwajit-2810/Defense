...
| Feature | What it does | State |
| ------- | ------------ | ----- |
| **Nine agents** | `analyst` (Q&A, budget 10), `coverage` (comment-coverage, 5), `alerting` (threshold monitoring, 8), `stance` (watchlist stance, 12), `comparator` (cross-campaign, 15), `toxicity` (harm patterns, 14), `narrative` (theme discovery, 12), `quality` (provenance & coverage audit, 8), `reporter` (report drafting, 15). All on the dedicated `agent` LLM role; corpus-tier only, never per post. The roster, tool allowlists and budgets are in [`registry.py`](src/defense/services/agents/registry.py) — that file is the source of truth, not this row. | 🟡 Works, unmeasured |
