...
def _max_continuations() -> int:
    ...
    """
    try:
        return max(0, config.llm_max_continuations)
    except ValueError:
        return 2
...
class LLMClient:
    ...
    def __init__(self) -> None:
        ...
        local_base_url: str = config.local_llm_base_url
        local_api_key: str = config.local_llm_api_key
        ...
        self._local_models: dict[str, str] = {
            role: getattr(config, _ROLE_LOCAL_ENV[role].lower(), _ROLE_LOCAL_DEFAULT[role])
            for role in VALID_ROLES
        }
        self._groq_models: dict[str, str] = {
            role: getattr(config, _ROLE_GROQ_ENV[role].lower(), _ROLE_GROQ_DEFAULT[role])
            for role in VALID_ROLES
        }
        ...
        cb_threshold = config.llm_cb_failure_threshold
        cb_cooldown = config.llm_cb_cooldown_seconds
        ...
        **Every successful call is recorded** in the Redis usage counters
        ...
        try:
            ...
            if effective_backend == "groq":
                log.warning(
                    "llm_groq_failed_falling_back_to_local role=%s model=%s error=%s",
                    role, model_id, exc,
                )
                effective_backend = "local"
                model_id = self._resolve_model(role, "local")
                ...
                try:
                    ...
                    log.error(
                        "llm_local_fallback_failed role=%s model=%s error=%s",
                ...
                log.error(
                    "llm_call_failed backend=%s role=%s model=%s error=%s",
        ...
        if response_format is not None and _is_degenerate_json(choice.message.content):
            log.warning(
                "llm_json_mode_degenerate_retrying_unconstrained backend=%s role=%s model=%s",
            ...
            try:
                ...
                if effective_backend == "groq":
                    log.warning(
                        "llm_json_retry_groq_failed_falling_back_to_local role=%s model=%s error=%s",
                        role, model_id, exc,
                    )
                    effective_backend = "local"
                    model_id = self._resolve_model(role, "local")
                    try:
                        ...
                        self._breakers["local"].record_failure()
                        log.error(
                            "llm_json_retry_local_fallback_failed role=%s model=%s error=%s",
                            role, model_id, local_exc,
                        )
                        raise
                    self._breakers["local"].record_success()
                    ...
                    log.error(
                        "llm_json_retry_failed backend=%s role=%s model=%s error=%s",
                        effective_backend, role, model_id, exc,
                    )
                    raise
            choice = completion.choices[0]
        ...
        log.info(
            "llm_call backend=%s role=%s model=%s tokens=%s latency_ms=%s "
            "finish_reason=%s truncated=%s continuations=%s",
            effective_backend, role, completion.model, totals["total_tokens"], latency_ms,
            finish_reason, truncated, continuations,
        )
        if truncated:
            log.warning(
                "llm_response_truncated backend=%s role=%s model=%s max_tokens=%s chars=%s",
    ...
    async def chat_stream(
        ...
        try:
            ...
            if effective_backend == "groq":
                log.warning(
                    "llm_stream_groq_open_failed_falling_back_to_local model=%s error=%s",
                    model_id, exc,
                )
                effective_backend = "local"
                model_id = self._resolve_model(role, "local")
                ...
                try:
                    ...
                    self._breakers["local"].record_failure()
                    log.error(
                        "llm_stream_local_fallback_failed model=%s error=%s",
                        model_id, local_exc,
                    )
                    yield {"type": "error", "error": str(local_exc)}
                    return
            else:
                log.error("llm_stream_open_failed", backend=effective_backend, error=str(exc))
        ...
        try:
            ...
            self._breakers[effective_backend].record_failure()
            log.error("llm_stream_failed", backend=effective_backend, model=model_id, error=str(exc))
            yield {"type": "error", "error": str(exc)}
            return

        latency_ms = round((time.perf_counter() - t0) * 1000.0, 1)
        log.info(
            "llm_stream backend=%s role=%s model=%s tokens=%s latency_ms=%s",
    ...
    async def list_models(self, backend: str) -> list[str]:
        ...
        try:
            ...
            log.warning("llm_list_models_failed", backend=backend, error=str(exc))
            return []
