...
_DISABLED = config.llm_usage_tracking_disabled
...
def _default_redis() -> Optional[Any]:
    ...
    if _shared_redis is None:
        try:
            ...
            log.warning("usage tracking unavailable (no redis)", error=str(exc))
            return None
...
async def track_usage(
    ...
    *,
    ...
    try:
        ...
        log.warning("usage_tracking_failed", error=str(exc))
