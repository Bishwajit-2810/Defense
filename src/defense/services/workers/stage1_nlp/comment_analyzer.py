...
from defense.libs.stance_scoring import aggregate_target_stances, score_comment_deterministic
from defense.libs.stance_targets import Targets, load_targets, unmatched_targets
...
_LLM_BATCH_RETRIES = max(0, config.stage1_llm_batch_retries)
...
def get_targets() -> Targets:
    ...
    if _TARGETS_CACHE is None:
        try:
            ...
            logger.error("stance watchlist failed to load", path=_STANCE_TARGETS_PATH, error=str(exc))
...
_LAUGH_SENTIMENT = config.comment_laugh_sentiment.lower()
    ...
    async def _run_batch(index: int, batch: list[dict]) -> int:
        ...
        if progress_cb is not None:
            try:
                ...
                logger.debug("comment batch progress callback failed", error=str(exc))
        return applied
...
async def analyze_comments(
    ...
    if substantive_idx:
        try:
            ...
            logger.warning("batched comment sentiment failed, falling back", error=str(exc))
    ...
    for comment in comments:
        ...
        try:
            ...
            logger.warning("Comment NLP failed", id=comment.get("id"), error=str(exc))
            nlp = {
                "sentiment": "neutral",
                "sentiment_score": 0.0,
                "emotion": "neutral",
