...
from defense.libs.common.utils import (  # noqa: E402
    compute_coverage,
    coverage_anomaly,
    platform_from_url,
...
CONSUMER_NAME = config.nlp_stage1_consumer or f"stage1-nlp-{os.getpid()}"
BLOCK_MS = int(config.redis_block_ms)
...
SENTIMENT_MODEL_CONFIG_KEY = config.sentiment_model_config_key
...
LLM_BACKEND_CONFIG_KEY = config.llm_backend_key

# Bounded retry before a failed message is dead-lettered to nlp:stage1:queue:dlq.
STAGE1_MAX_RETRIES = int(config.stage1_max_retries)
...
_OCR_SENTIMENT = config.stage1_ocr_sentiment
...
def _build_result(
    ...
    return {
        ...
        "processing": {
            ...
            "stub_mode": config.model_stub_mode,
