...
async def analyze_image(
    ...
    try:
        ...
        logger.error("Failed to fetch image", url=url, error=str(exc))
    ...
    if processor is None or model is None:
        logger.warning("CLIP/SigLIP unavailable; no image sentiment", url=url)
        return _absent_vision_result(STATUS_MODEL_UNAVAILABLE, ocr_text)

    try:
        label, score = _clip_sentiment(image_bytes, processor, model)
    except Exception as exc:
        logger.warning("CLIP sentiment failed", url=url, error=str(exc))
