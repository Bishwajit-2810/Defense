...
from defense.services.api.deps import get_current_user, get_db, get_redis

from defense.libs.llm.usage import ALL_LANES, PIPELINE_LANES
...
def _price_per_1k(backend: str, model: str) -> float:
    ...
    config = get_settings()
    override = config.get_groq_price_override(model)
